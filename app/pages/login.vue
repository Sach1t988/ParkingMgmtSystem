...
async function submit() {
  ...
  try {
    ...
    toast.add({
      title: "Login failed",
      description: "Use valid administrator credentials.",
      color: "error",
    });
  } finally {
    pending.value = false;
...
<template>
  <main class="min-h-screen bg-gray-50 flex items-center justify-center p-6">
    <UCard class="w-full max-w-md">
      <template #header
        ><h1 class="text-xl font-semibold">Parking Management</h1></template
      >
          ...
          ><UInput
            ...
            ><template #trailing
              ><UButton
                ...
                color="neutral"
                :padded="false"
                @click="showPassword = !showPassword" /></template></UInput
        ></UFormField>
